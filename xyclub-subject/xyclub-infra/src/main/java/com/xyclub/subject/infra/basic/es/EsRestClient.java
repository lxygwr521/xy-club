...
/**
 * ES RestHighLevelClient 工具封装，按集群名称缓存客户端并提供常用文档操作。
 */
@Component
@Slf4j
public class EsRestClient {

    public static final Map<String, RestHighLevelClient> CLIENT_MAP = new HashMap<>();
    ...
    public void initialize() {
        ...
        for (EsClusterConfig esConfig : esConfigs) {
            ...
            if (restHighLevelClient != null) {
                CLIENT_MAP.put(esConfig.getName(), restHighLevelClient);
            } else {
    ...
    /**
     * 执行自定义 Bool 查询，支持字段过滤、分页、高亮、排序和 scroll。
     */
    public static SearchResponse searchWithTermQuery(EsIndexInfo esIndexInfo, EsSearchRequest esSearchRequest) {
        try {
            ...
            // from/size 控制分页，fetchSource 控制返回字段范围。
            ...
            if (Boolean.TRUE.equals(needScroll)) {
                // scroll 适合大批量遍历，minutes 表示 ES 保留快照上下文的时间。
